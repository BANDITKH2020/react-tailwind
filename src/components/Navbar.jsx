    ...
    return (
        <>
                        ...
                        <ul className='hidden md:flex space-x-12'>
                            {
                                ...
                                key={link} to={path} className='block hover:text-gray-300 cursor-pointer'>{link}</Link>)
                            }
                        </ul>
                    </div>
            ...
            <div className={`space-y-4 px-4 pt-24 pb-5 bg-secondary text-xl ${isMenuOpen ? "block fixed top-0 right-0 left-0" : "hidden"}`} >
                {
                    navItems.map(({link, path}) => <Link activeClass='active' spy={true} smooth={true} offset={-90} 
                    key={link} to={path} className='text-white block hover:text-gray-300 cursor-pointer' onClick={toggleMenu}>{link}</Link>)
                }
